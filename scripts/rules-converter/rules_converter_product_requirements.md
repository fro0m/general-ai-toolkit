...
*   **Purpose**: The Rules Converter is a multi-stage command-line tool that processes template-based rule files through variable substitution, validation, and format conversion to generate ready-to-use rules for different AI-powered code editors and assistants. This ensures that a single source of truth for coding rules and instructions can be parameterized and deployed across multiple development environments.
...
*   **Scope**: This document covers the 4-stage processing pipeline of the Rules Converter: template variable substitution, validation, format conversion, and deployment to target directories.
...
*   **Input**: `cooked_rules_template/` directory (follows Cursor IDE rules format)
*   **Process**: Convert files to target formats for different AI tools
...
*   **Format Conversion**:
    *   Convert from Cursor IDE format to VS Code instruction files (`.instructions.md`)
...
*   **Must-have**: 
    ...
    *   Conversion to all five target formats
...
./rules-converter.py /path/to/source/directory /path/to/rules-description.json --output /custom/output/path
...
```json
{
  "DartboardID": "MyOffice/Tasks",
    ...
    "ApplicationName": "QtDesktopStorybook",
    "GeneralAIToolkitPath": "/home/dev/dev/prj/general-ai-toolkit",
    "ProjectSpecificAIToolkitPath": "/home/dev/dev/prj/project-ai-toolkit"
}
```
