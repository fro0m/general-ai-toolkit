...
```
poetry run python auto_inserter/main.py
```

To run it in the background:

```
nohup poetry run python auto_inserter/main.py &
```
...
[Service]
Type=simple
ExecStart=/usr/bin/env bash -c 'cd /path/to/auto-inserter && poetry run python auto_inserter/main.py'
Restart=always
RestartSec=5
